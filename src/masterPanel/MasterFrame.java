...
public class MasterFrame extends JFrame
{
    ...
    class MouseMotionListenerFrame implements MouseMotionListener
    {
        ...
        public void mouseDragged(MouseEvent e)
        {
            ...
            {
                int stepX = e.getX() - MasterFrame.this.begin.x;
                newSize=MRB.isDesc? new Dimension(MasterFrame.this.getPreferredSize().width + stepX,
                        ...
                        new Dimension(MasterFrame.this.getPreferredSize().width + stepX,
                                MasterFrame.this.getPreferredSize().height )
                        ;
            }
            ...
            {
                int stepY = e.getY() - MasterFrame.this.begin.y;
                newSize =MRB.isDesc? new Dimension(MasterFrame.this.getPreferredSize().width + MasterFrame.this.MRB.BR.getStepX(stepY),
                        MasterFrame.this.getPreferredSize().height + stepY):
                        new Dimension(MasterFrame.this.getPreferredSize().width ,
                        MasterFrame.this.getPreferredSize().height + stepY)
                        ;
    ...
    public ChangeListener CL_transparency = new ChangeListener()
    {
        ...
        public void stateChanged(ChangeEvent e)
        {
            ...
            if (source.getValue() > MaxValueOpacity)
            {
                source.setValue(MaxValueOpacity);
                return;
            }
            float transp = (float)(1 - (float)source.getValue() /(float) source.getMaximum());
    ...
    private final int toolPanelWidth = 220;
    private final int toolPanelX = 100;
    private MasterReceiverBoard MRB;
    private ReceiverBoard_UDP receiver;
    private ReceiverScreeen_UDP screen;
    private ConnectionManager SenderPrScr;
    ...
    public MasterFrame()
    {
        ...
        this.MRB.tools.connectionStatus.addActionListener(new ActionListener()
        {
            ...
            {               
                ...
                {
                    if(CD.getValue()>0 && f)
                    JOptionPane.showMessageDialog(MasterFrame.this, "Изменения вступят в силу после перезагрузки приложения");
                }
            }
        });
        ...
        this.addWindowListener(new WindowAdapter()
        {
            ...
            {
                SC.saveBounds(MasterFrame.this.getBounds(),  AWTUtilities.getWindowOpacity(((Window) MasterFrame.this)));
                System.exit(0);
            }
        ...
        try
        {
            ...
            try{
            AWTUtilities.setWindowOpacity(this, SC.opacity);
            }
            catch (Exception ex){}
        } catch (Exception ex)
        {
        ...
        this.tools.trancparency.setValue((int)((1 - SC.opacity) *this.tools.trancparency.getMaximum()));
        if(this.isConnect)
        { 
            startConnection();
        }
    ...
    {
        SenderPrScr.startConnection();
        receiver.start();
        MRB.tools.connectionStatus.setIcon(ImageIconURL.get("resources/IP_student_On.png"));    
    }
    ...
    private void setFrameSize()
    {
        ...
        else
        {
            ...
            d = new Dimension(MasterFrame.this.MRB.TP.imgD.width, MasterFrame.this.MRB.TP.imgD.height+MasterFrame.this.MRB.tools.getHeight());
        }
        if (d.height!=this.getHeight() ||d.width!=this.getWidth())
        {           
            this.setSize(d);
